...
SCRIPT = Path(__file__).with_name("generate_barbell.py")
SPEC = importlib.util.spec_from_file_location("generate_barbell", SCRIPT)
assert SPEC and SPEC.loader
generator = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(generator)


class GenerateBarbellTest(unittest.TestCase):
    def test_generates_a_square_png_with_the_requested_background(self) -> None:
        self.assertEqual(generator.DEFAULT_SOURCE.name, "bbell2_cropped.png")
